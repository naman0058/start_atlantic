...
$.getJSON(`/api/get-tour`, data => {
    addgroup = data
    console.log('leagues',data)
    fillDropDown('cityid', data, 'Choose Tour', 0)
    ...
    <div class="col">
      <div class="card bg-default shadow">
        <div class="card-header bg-transparent border-0">
          <h3 class="text-white mb-0">All Trip plan</h3>
          <br>
            ...
            <tbody>
                  ...
                  $.each(board, function(i, item) {
                    ...
                    <th scope="row">
                      <div class="media align-items-center">
                        <div class="media-body">
                          <span class="mb-0 text-sm">${item.projectname}</span>
                        </div>
                      </div>
                    </th>
                    ...
                    <th scope="row">
                      <div class="media align-items-center">
                        <div class="media-body">
                          <span class="mb-0 text-sm">${item.day}</span>
                        </div>
                      </div>
                    </th>
                    ...
                    <th scope="row">
                      <div class="media align-items-center">
                        <div class="media-body">
                          <span class="mb-0 text-sm">${item.cityname}</span>
                        </div>
                      </div>
                    </th>
...
$('#result').on('click', '.edit', function() {
    ...
    fillDropDown('pcityid', addgroup, 'Choose Tour', result.projectid)
    $('#editdiv').show()
    $('#insertdiv').hide() 
    $('#result').hide()
    $('#pid').val(result.id)
    $('#pday').val(result.day)
    $('#pcityname').val(result.cityname)
    $('#pshort_description').val(result.short_description)
    $('#pshort_description1').val(result.short_description1)
...
$('#update').click(function(){  //data insert in database
    ...
    let updateobj = {
        ...
        day:$('#pday').val(),
        cityname:$('#pcityname').val(),
        short_description:$('#pshort_description').val(),
        short_description1:$('#pshort_description1').val(),
